setwd("/Users/rt485/Downloads/Helfer/Phenotypic_indices/yield/New\ GDD_9_13/GNDVI/2-4-2023/newGDD/growth\ curve_stability/NDRE")

d350 <- read.table("gv_NDRE_350_new_time_3_27_a.txt", header = T)
d350$Harvest <- "2020cut1"

d389 <- read.table("gv_NDRE_389_new_time_3_27_a.txt", header = T)
#d389$Harvest <- "Second Harvest of 2020"
d389$Harvest <- rep("2020cut2", nrow(d389))

d438 <- read.table("gv_NDRE_438_new_time_3_27_a.txt", header = T)
#d438$cutting <- "Third Harvest of 2020"
d438$Harvest <- rep("2020cut3", nrow(d438))

d729 <- read.table("gv_NDRE_729_new_time_3_27_a.txt", header = T)
d729$Harvest <- rep("2021cut1", nrow(d729))

d772 <- read.table("gv_NDRE_772_new_time_3_27_a.txt", header = T)
d772$Harvest <- rep("2021cut2", nrow(d772))

d821 <- read.table("gv_NDRE_821_new_time_3_27_a.txt", header = T)
